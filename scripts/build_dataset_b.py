...
OUT = os.path.join(HERE, "data", "targets_b")
...
COFACTORS = {
    ...
    "CHL", "CLA", "BCB", "HEM", "HEC", "HEA", "HEB", "HDD", "DHE", "VER", "SRM", "COH", "HEV", "MHM",
...
def main():
    ...
    ids = json.load(open(os.path.join(HERE, "data", "candidate_ids_b.json")))
    limit = int(sys.argv[1]) if len(sys.argv) > 1 else 400
    want = int(sys.argv[2]) if len(sys.argv) > 2 else 80
    kept, tried, seen_unp = [], 0, set()
    mpath = os.path.join(HERE, "data", "manifest_b.json")
    if os.path.exists(mpath):
        kept = json.load(open(mpath))
        for k in kept:
            seen_unp |= set(k.get("uniprot", []))
    done = {f"{k['pdb']}_{k['chain']}" for k in kept}
    print(f"resuming with {len(kept)} already built", flush=True)
    for pdb_id in ids[:limit]:
        tried += 1
        if any(d.startswith(pdb_id + "_") for d in done):
            continue
        try:
            r = build_one(pdb_id)
        except Exception:
        ...
        json.dump(kept, open(os.path.join(HERE, "data", "manifest_b.json"), "w"), indent=1)
        if len(kept) >= want:
            break
    json.dump(kept, open(os.path.join(HERE, "data", "manifest_b.json"), "w"), indent=1)
