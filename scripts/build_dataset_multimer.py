...
OUT = os.path.join(HERE, "data", "targets_multimer")
...
MAX_LEN = 820          # whole assembly, so the cap is higher
MIN_CHAINS = 2         # the point of this set: keep the oligomer intact
...
def build_one(pdb_id: str):
    """Keep the whole multi-chain assembly as one graph, not a single chain."""
    text = fetch(pdb_id)
    if not text:
        return None
    residues, order, ligands = parse(text)
    if len(residues) < MIN_CHAINS:
        return None

    cb_all, heavy_all, res_all, chain_of = [], [], [], []
    for chain in sorted(residues):
        arr = chain_arrays(residues, order, chain)
        if arr is None:
            continue
        cb_c, heavy_c, res_c = arr
        cb_all.append(cb_c)
        heavy_all.extend(heavy_c)
        res_all.append(res_c)
        chain_of.extend([chain] * len(cb_c))
    if len(cb_all) < MIN_CHAINS:
        return None
    cb = np.concatenate(cb_all)
    resnums = np.concatenate(res_all)
    n = len(cb)
    ...
    for (chain, resname, rkey), d in ligands.items():
        ...
        if len(xyz):
            ligs.append((resname, rkey, xyz, set(d["elem"])))
    cofs = [l for l in ligs if l[0] in COFACTORS]
    drugs = [l for l in ligs
             if l[0] not in COFACTORS and len(l[2]) >= MIN_HEAVY
             and (l[3] & DRUGLIKE_ELEMS)]
    if not cofs or not drugs:
        return None

    anchor = np.unique(np.concatenate([contacts(heavy_all, l[2]) for l in cofs]))
    if len(anchor) < MIN_SITE:
        return None

    from scipy.spatial.distance import cdist
    best = None
    for resname, rkey, xyz, _e in drugs:
        y_idx = contacts(heavy_all, xyz)
        if len(y_idx) < MIN_SITE or np.intersect1d(y_idx, anchor).size:
            continue
        ...
        y[y_idx] = 1
        cand = dict(pdb=pdb_id, chain="+".join(sorted(set(chain_of))), ligand=resname,
                    n=n, cb=cb, anchor=anchor, y=y, resnums=resnums,
                    n_anchor=len(anchor), n_pos=int(y.sum()), sep=float(sep),
                    n_chains=len(set(chain_of)),
                    chain_id=np.array(chain_of, dtype="U4"))
...
def main():
    ...
    ids = json.load(open(os.path.join(HERE, "data", "candidate_ids_multimer.json")))
    limit = int(sys.argv[1]) if len(sys.argv) > 1 else 400
    want = int(sys.argv[2]) if len(sys.argv) > 2 else 80
    kept, tried, seen_unp = [], 0, set()
    done = {f.split("_")[0] for f in os.listdir(OUT) if f.endswith(".npz")}
    ...
    for pdb_id in ids[:limit]:
        ...
        path = os.path.join(OUT, f"{r['pdb']}_multi.npz")
        np.savez_compressed(path, cb=r["cb"], anchor=r["anchor"], y=r["y"],
                            resnums=r["resnums"], chain_id=r["chain_id"])
        kept.append({k: r[k] for k in ("pdb", "chain", "ligand", "n", "n_anchor",
                                       "n_pos", "sep", "uniprot", "n_chains")})
        json.dump(kept, open(os.path.join(HERE, "data", "manifest_multimer.json"), "w"),
                  indent=1)
        print(f"[{len(kept):3d}/{tried}] {r['pdb']}_{r['chain']} lig={r['ligand']:>4s} "
              f"N={r['n']:4d} ch={r['n_chains']} anchor={r['n_anchor']:3d} pos={r['n_pos']:3d} "
              f"sep={r['sep']:.1f}A", flush=True)
        if len(kept) >= want:
            break
    json.dump(kept, open(os.path.join(HERE, "data", "manifest_multimer.json"), "w"), indent=1)
